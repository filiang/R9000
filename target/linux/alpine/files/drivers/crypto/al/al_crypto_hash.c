		...
		if (unlikely(in_len == 0)) {
			if (!ctx->sa.auth_hmac_en)
				...
				rc = crypto_shash_init(&desc.shash) ? :
					crypto_shash_import(&desc.shash, ipad) ? :
					crypto_shash_final(&desc.shash, req->result) ? :
					crypto_shash_import(&desc.shash, opad) ? :
					crypto_shash_finup(&desc.shash, req->result,
